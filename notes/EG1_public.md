...
In-paper anchor: `paper/SCHINZEL_HYPOTHESIS_H_PREPRINT.md` (`SHH_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving simultaneous prime values for admissible polynomial families through an admissible polynomial-pattern closure architecture`.
...
- projected core: the response sector controlled by `kappa_pattern`.
...
`SHH_G1` closes when `kappa_pattern` satisfies the response-floor requirement: projected polynomial-pattern response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_SHH`.
...
Payload: verify that all quantities used by `kappa_pattern` are defined on the projected sector named in the main preprint.
...
Payload: check the artifact key `kappa_pattern` and the corresponding extraction input/provenance record.

### Theorem EG1.3 (core gate closure)
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_pattern`, then `SHH_G1` supplies the projected/protected-core input to `M_SHH`.

## Current Instantiation

- gate: `SHH_G1`
- artifact key: `kappa_pattern`
