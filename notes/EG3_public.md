...
In-paper anchor: `paper/SCHINZEL_HYPOTHESIS_H_PREPRINT.md` (`SHH_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving simultaneous prime values for admissible polynomial families through an admissible polynomial-pattern closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`SHH_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and congruence windows do not collapse.
This is the first-failure capture contribution to `M_SHH`.
...
If near-failure extraction and no-Zeno spacing hold, then `SHH_G3` prevents hidden drift before rigidity is applied.
