...
Canonical Lane research workspace for a flagship problem in prime-distribution theory:
proving simultaneous prime values for admissible polynomial families through an admissible polynomial-pattern closure architecture.

## Main Manuscript

- [paper/SCHINZEL_HYPOTHESIS_H_PREPRINT.md](paper/SCHINZEL_HYPOTHESIS_H_PREPRINT.md)
- [paper/CANONICAL_ROUTING_INDEX.md](paper/CANONICAL_ROUTING_INDEX.md)

## Structure

- `paper/`
  - `SCHINZEL_HYPOTHESIS_H_PREPRINT.md`
...
1. Theorem chain first: read `paper/SCHINZEL_HYPOTHESIS_H_PREPRINT.md`.
...
Current SHH runner policy:

- `repro/run_repro.sh` extracts, promotes, runs `scripts/shh_closure_guard.py`, updates `repro/repro_manifest.json`, and enforces `fully_extracted` release-gate mode.
...
3. artifact key consumed by `scripts/shh_closure_guard.py`.
...
- Manuscript target: [paper/SCHINZEL_HYPOTHESIS_H_PREPRINT.md](paper/SCHINZEL_HYPOTHESIS_H_PREPRINT.md)
