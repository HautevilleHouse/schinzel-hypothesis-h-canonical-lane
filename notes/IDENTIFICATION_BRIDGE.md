...
In-paper anchor: `paper/SCHINZEL_HYPOTHESIS_H_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving simultaneous prime values for admissible polynomial families through an admissible polynomial-pattern closure architecture`.
...
- endpoint lock: `pattern_transfer`.
...
2. the endpoint-transfer lock `pattern_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_SHH` remains positive.
...
Payload: verify `pattern_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_SHH` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `pattern_transfer`
